...
// --- Main Application ---
int main() {
    InitWindow(1200, 800, "Ephemeris Live - Pro");
    SetTargetFPS(60);
    ApplyStyle();
    LoadSpecs();
    ...
    while (!WindowShouldClose()) {
        ...
        if (state.current_tab == 0) { // PLANETARIUM
            DrawRectangleRec({ 260, 20, 760, 500 }, { 20, 20, 20, 255 });

            // Polar Projection Map Background
            Vector2 mapCenter = { 340 + 600 / 2.0f, 20 + 500 / 2.0f };
            ...
            if (state.isFetching) {
                DrawRectangleRec({ 260, 20, 760, 500 }, Fade(BLACK, 0.8f)); // Darkens the map
                ...
                DrawText(loadingStr.c_str(), 340 + (600 - textW) / 2, 260, 20, ACCENT);
            }

            DrawRectangleLinesEx({ 260, 20, 760, 500 }, 2, ACCENT); // Outer border

            // Target Selector Panel
            GuiGroupBox(Rectangle{ 20, 20, 220, 500 }, "SELECT TARGET");
            int btnY = 50;
            for (auto& p : state.planets) {
                bool isTarget = p.selected;
                DrawCircle(40, btnY + 15, 5, p.isVisible && p.dataLoaded ? GREEN : RED);
                GuiToggle(Rectangle{ 60, (float)btnY, 160, 30 }, p.name.c_str(), &isTarget);
